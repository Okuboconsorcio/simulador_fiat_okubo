...
class TestSaldoDevedorExtratosReais(unittest.TestCase):
    ...
    def test_diferenca_mpm_paga_com_recursos_proprios_abate_proporcional(self):
        # Cláusula 3.4, inciso II: quita-se a diferença "recolhida a menor
        # antes da contemplação" — proporcional aos meses pagos, não os 25%
        # do crédito. No mês 6: 6 x 0,321875% = 1,93125 pontos.
        base = caso_thiago()
        pago = caso_thiago(
        ...
        self.assertEqual(diferenca, Decimal("6") * Decimal("0.00321875"))
        # Valor em reais a desembolsar: 120.000 x 1,93125% = 2.317,50
        self.assertEqual(pago.diferenca_paga_recursos_proprios, Decimal("2317.50"))

    def test_diferenca_mpm_deduzida_do_credito(self):
        # Inciso III é a exceção: 25% FIXOS do crédito ("será disponibilizado
...
class TestReduzirPrazoOrdemInversa(unittest.TestCase):
    ...
    def test_mpm_dilui_apenas_diferenca_proporcional_no_reduzir_prazo(self):
        # Contemplação na 1ª assembleia (crédito 70.000): a diferença é de UMA
        # parcela (R$ 225,31), diluída em 60 vincendas = +R$ 3,76/mês.
        # Parcela: 1.076,25 (plena) + 3,76 + 52,52 (seguro) = 1.132,53.
        resultado = caso_thiago(
            credito=Decimal("70000"), mes_contemplacao=1, uso_lance="REDUZIR_PRAZO"
        )
        amortizacao = calcular_reduzir_prazo(resultado, 1)
        self.assertEqual(amortizacao.parcelas_restantes, Decimal("60"))
        self.assertLessEqual(abs(amortizacao.nova_parcela - Decimal("1132.53")), Decimal("0.02"))
